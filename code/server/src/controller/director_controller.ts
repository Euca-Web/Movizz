...
import directorRepository from "../repository/director_repository.js";

class directorController {
	public index = async (req: Request, res: Response) => {
		// récuperer tous les enregistrements
		const result = await new directorRepository().selectAll();
	...
	public one = async (req: Request, res: Response) => {
		...
		const result = await new directorRepository().selectOne(req.params);
		// Si la requête SQL renvoie une erreur
		if(result instanceof Error){
			res.status(400).json({
				status: 400,
