...
class movies_genderRepository {
	...
	public selectAll = async (): Promise<movies_gender[] | unknown> => {
		...
		try {
			...
			for (let i = 0; i < (results as movies_gender[]).length; i++) {
				const result = (results as movies_gender[])[i];
				result.movie = await new movies_genderRepository().selectOne({
	...
	public selectOne = async (
		data: Partial<movies_gender>,
	): Promise<movies_gender[] | unknown> => {
		const connection = await new MySqlService().connect();
		// console.log(connection);
		...
		try {
			...
			const result = (results as movies_gender[]).shift() as movies_gender;
